...
// Case: Float base with a negative exponent -- lowered via x^(-n) == (1/x)^n.
// The base is inverted and the exponent negated under the same isNeg flag,
// then the regular exp-by-squaring loop runs on (1/x, |n|). No assert is
// emitted on this path.
// CHECK-LABEL: @pow_float_base_neg_exp
func.func @pow_float_base_neg_exp(%a: f32) -> f32 {
  %c = arith.constant -3 : i32
  // CHECK: %[[NEG:.*]] = arith.cmpi slt
  // CHECK: %[[RECIP:.*]] = arith.divf
  // CHECK: %[[BASE:.*]] = arith.select %[[NEG]], %[[RECIP]]
  // CHECK: arith.subi
  // CHECK: %[[EXP:.*]] = arith.select %[[NEG]]
  // CHECK-NOT: cf.assert
  // CHECK: scf.for {{.*}} iter_args({{.*}} = {{.*}}, {{.*}} = %[[BASE]], {{.*}} = %[[EXP]])
  %0 = aadesh.pow %a, %c : (f32, i32) -> f32
  return %0 : f32
}
